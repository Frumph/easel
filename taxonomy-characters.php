	...
	if ($insertPage->have_posts()) {
		...
		if (!empty($qposts)) {
			...
			if ($first_seen_id == $last_seen_id) {
				$output .= '<i>'.__( 'Only Appearance:', 'easel' ).'</i> <a href="'.get_permalink($first_seen_id).'">'.$first_seen_title.'</a><br />';
			} else {
				$output .= '<i>'.__( 'Recent Appearance:', 'easel' ).'</i> <a href="'.get_permalink($last_seen_id).'">'.$last_seen_title.'</a><br />';
				$output .= '<i>'.__( 'First Appearance:', 'easel' ).'</i> <a href="'.get_permalink($first_seen_id).'">'.$first_seen_title.'</a><br />';			
			}
			$output .= '</div>';
			echo apply_filters('easel-character-stats', $output);
		}		
