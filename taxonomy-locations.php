...
if (!empty($location)) {
	...
	if (!empty($qposts)) {
		...
		if ($first_seen_id == $last_seen_id) {
			$cast_output .= '<i>'.__( 'Only Scene:', 'easel' ).'</i> <a href="'.get_permalink($first_seen_id).'">'.$first_seen_title.'</a><br />';
		} else {
			$cast_output .= '<i>'.__( 'Recent Scene:', 'easel' ).'</i> <a href="'.get_permalink($last_seen_id).'">'.$last_seen_title.'</a><br />';
			$cast_output .= '<i>'.__( 'First Scene:', 'easel' ).'</i> <a href="'.get_permalink($first_seen_id).'">'.$first_seen_title.'</a><br />';			
		}
		$cast_output .= '</div>';
		echo $cast_output;
	}
