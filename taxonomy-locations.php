...
if (!empty($location)) {
	...
	if (!empty($qposts)) {
		...
		if ($first_seen_id == $last_seen_id) {
			$output .= '<i>'.__( 'Only Scene:', 'easel' ).'</i> <a href="'.get_permalink($first_seen_id).'">'.$first_seen_title.'</a><br />';
		} else {
			$output .= '<i>'.__( 'Recent Scene:', 'easel' ).'</i> <a href="'.get_permalink($last_seen_id).'">'.$last_seen_title.'</a><br />';
			$output .= '<i>'.__( 'First Scene:', 'easel' ).'</i> <a href="'.get_permalink($first_seen_id).'">'.$first_seen_title.'</a><br />';			
		}
		$output .= '</div>';
		echo $output;
	}
